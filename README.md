...
Now that the table is created, and the required assets are uploaded to our portal, we'll need to add the Recruiting Events module to our Role Listings page. Go ahead and edit the Role Listings page we created earlier, and drag and drop the Recruiting Events module below the Role Listing module. This module will list all rows of the HubDB table we created, but it is possible to apply filtering and ordering in a similar fashion to the other GraphQL queries in this project.
